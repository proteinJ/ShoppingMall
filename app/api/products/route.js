    ...
    try {
      ...
      const products = await prisma.product.findMany({
        ...
        if (products) {
          ...
          return NextResponse.json({ success: true, message: "상품 목록 정보 없음", data: [] }, { status: 404 });
        }
          
    } catch (error) {
        console.log(`[${new Date().toISOString()}] [ERROR] ❌ 상품 목록 조회 로직 실패 - error ${error}`);
        return NextResponse.json({ success: false, message: "상품 목록 조회 로직 실패", data: [] }, { status: 500 });
    } finally {
